...
public class CompanyDetailsService implements UserDetailsService {
    ...
    public CompanyDetails loadUserByUsername(String rfc){
        return new CompanyDetails(companyRepository.findByRfc(rfc).orElseThrow(()-> new EntityNotFoundException(ExceptionMessage.ENTITY_NOT_FOUND.format(Report.COMPANY.getName(), rfc))));
    }
}
